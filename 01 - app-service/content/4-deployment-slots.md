...
- General settings, such as framework version, 32/64-bit, web sockets are swapped.
- App settings (can be configured to stick to a slot) are swapped.
- Connection strings (can be configured to stick to a slot) are swapped.
- Handler mappings are swapped.
- Public certificates are swapped.
- WebJobs content are swapped.
- Hybrid connections * are swapped.
- Azure Content Delivery Network * are swapped.
- Service endpoints * are swapped.
- Path mappings are swapped.
- Managed identities are swapped.
- Settings that end with the suffix _EXTENSION_VERSION are swapped.

### Settings that are not swapped

- Publishing endpoints are not swapped.
- Custom domain names are not swapped.
- Non-public certificates and TLS/SSL settings are not swapped.
- Scale settings are not swapped.
- WebJobs schedulers are not swapped.
- IP restrictions are not swapped.
- Always On are not swapped.
- Diagnostic log settings are not swapped.
- Cross-origin resource sharing (CORS) are not swapped.
- Virtual network integration are not swapped.

To make settings that are not swapped by default swappable, add the app setting WEBSITE_OVERRIDE_PRESERVE_DEFAULT_STICKY_SLOT_SETTINGS in every slot of the app and set its value to 0 or false. These settings are either all swappable or not at all. You can't make just some settings swappable and not the others. Managed identities are never swapped and are not affected by this override app setting.
...
Go to your app's resource page and select Deployment slots. In the Traffic % column of the slot you want to route to, specify a percentage (between 0 and 100) to represent the amount of total traffic you want to route. Select Save.
