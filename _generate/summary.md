...
Microsoft Identity Platform's sign-in endpoint is "/.auth/login/aad"
Facebook's sign-in endpoint is "/.auth/login/facebook"
Google's sign-in endpoint is "/.auth/login/google"
Twitter's sign-in endpoint is "/.auth/login/twitter"
Any OpenID Connect provider's sign-in endpoint is "/.auth/login/<providerName>"
GitHub's sign-in endpoint is "/.auth/login/github"
...
### Without the provider SDK

To sign user in without the provider SDK Redirects client to /.auth/login/<provider>.
Post-authentication without the provider SDK Provider redirects client to /.auth/login/<provider>/callback.
Establish authenticated session without the provider SDK App Service adds authenticated cookie to response.
Serve authenticated content without the provider SDK Client includes authentication cookie in subsequent requests (automatically handled by browser).

### With the proider SDK

Sign user in with the provider SDK - Client code signs user in directly with provider's SDK and receives an authentication token. For information, see the provider's documentation.
Post-authentication with the provider SDK - Client code posts token from provider to /.auth/login/<provider> for validation.
Establish authenticated session with the provider SDK - App Service returns its own authentication token to client code.
Serve authenticated content with the provider SDK - Client code presents authentication token in X-ZUMO-AUTH header (automatically handled by Mobile Apps client SDKs).
...
Some inbound features include App-assigned address, Access restrictions, Service endpoints, and Private endpoints.
Some outbound features include Hybrid connections, Gateway-rquired virtual network integration, virtual network integration. You can mix these features to solve your problems with a few exceptions. The following inbound use cases are examples of how to use App Service networking features to control traffic inbound to your app.

App-assigned address feature can be used to support IP-based SSL needs for your app and an unshared dedicated inbound address for your app.

Access restrictions can restrict access to your app from a set of well-defined addresses.
