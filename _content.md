...
APM tools monitor applications by proactively understand how an application is performing and reactively review application execution data to determine the cause of an incident.
...
“Synthetic Transaction Monitoring”, probes your applications external endpoint(s) to test the overall availability and responsiveness over time.
Use GitHub or Azure DevOps integration to create work items in context of Application Insights data.
Usage data helps one understand which features are popular with users and how users interact and use your application
Smart Detection facilitates automatic failure and anomaly detection through proactive telemetry analysis.
Application Map is a high level top-down view of the application architecture and has at-a-glance visual references to component health and responsiveness.
Distributed Tracing is used to search and visualize an end-to-end flow of a given execution or transaction.
...
There are two kinds of metrics: Log based metrics and Standard metrics.
...
You can create up to 100 availability tests per Application Insights resource, and there are three types of availability tests: Url ping test (classic), Standard test (Preview), Custom TrackAvailability test

You can create an URL ping test through the portal to validate whether an endpoint is responding and measure performance associated with that response. You can also set custom success criteria coupled with more advanced features, like parsing dependent requests and allowing for retries.

This Standard test (a single test) is similar to the URL ping test. It includes SSL certificate validity, proactive lifetime check, HTTP request verb (for example GET, HEAD, or POST), custom headers, and custom data associated with your HTTP request.
